...
export default function Projects() {
  ...
  const updateMcpPorts = async () => {
    ...
    const parts = mcpPortsInput
      ...
      .map((item) => item.trim())
      .filter(Boolean);
    const ports = parts.map((item) => Number(item)).filter((item) => !Number.isNaN(item));
    try {
  ...
  const modelOptionsFor = (providerName: string, roleName: string, currentValue: string) => {
    const filtered = models.filter((item) => item.provider === providerName).map((item) => item.id);
  ...
  return (
    <section>
      ...
      <div className="card">
        ...
        {orderedModelDefaults.map(([role, entry]) => (
          <div key={role} className="row" style={{ marginBottom: 6 }}>
            ...
            >
              {modelOptionsFor(entry.provider, role, entry.model).map((modelId) => (
      ...
      <div className="card">
        ...
        {orderedMemoryProfiles.map(([role, profile]) => (
          <div key={role} className="row" style={{ marginBottom: 6 }}>
            <span className="pill">{role}</span>
            <input
              type="number"
                ...
                <input
                  ...
                  onChange={(e) => {
                    const next = e.target.checked
                      ...
                      : enabledPlugins.filter((item) => item !== plugin.id);
