...
class JobEngine:
    ...
    def create_job(self, run_id: int) -> Job:
    ...
    async def run(
        ...
        for step_name in steps:
            step = self._start_step(job, step_name)
            await self._emit(job, "job.step.started", {"step": step_name, "job_id": job.id})
            result = await self._run_step_with_retries(step, handlers, max_attempts)
            if not result.success:
                self._fail_job(job, step, result.error or "step_failed")
                await self._emit(
                    job,
                    "job.failed",
                    {"step": step_name, "job_id": job.id, "error": result.error},
                )
                return
            self._complete_step(job, step)
            await self._emit(job, "job.step.completed", {"step": step_name, "job_id": job.id})

        self._complete_job(job)
        await self._emit(job, "job.completed", {"job_id": job.id})
    ...
    def _start_step(self, job: Job, name: str) -> JobStep:
        with get_session() as session:
            job.status = "running"
    ...
    def _complete_step(self, job: Job, step: JobStep) -> None:
        with get_session() as session:
    ...
    def _fail_job(self, job: Job, step: JobStep, error: str) -> None:
        with get_session() as session:
            persisted = session.get(JobStep, step.id)
            if persisted:
